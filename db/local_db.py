...
from .repositories import UserBehaviorRepository, EngineerRepository
...
class LocalUserBehaviorDB:
    ...
    def record_user_behavior(self, action_type: str, action_data: Optional[Dict[str, Any]] = None, 
                            engineer_id: Optional[int] = None, session_id: Optional[str] = None, 
                            user_id: str = 'default_user'):
        """兼容性方法：记录用户行为"""
        return self.repo.record_behavior(user_id, action_type, action_data, engineer_id, session_id)
    ...
    def create_recommendation(self, recommendation_type: str, content: str, 
                            engineer_id: Optional[int] = None, user_id: str = 'default_user'):
        """兼容性方法：创建推荐"""
        return self.repo.create_recommendation(user_id, recommendation_type, content, engineer_id)
...
class LocalEngineerDB:
    """
    技师数据库的兼容性类
    
    将旧接口重定向到新的EngineerRepository
    """
    
    def __init__(self, db_path='sqlite:///data/smart_appointment.db'):
        warnings.warn(
            "LocalEngineerDB已弃用，请使用 DatabaseRouter().engineers 或 EngineerRepository",
            DeprecationWarning,
            stacklevel=2
        )
        self.session_manager = SessionManager(db_path)
        self.repo = EngineerRepository(self.session_manager)

    def get_engineer_by_name(self, session, name: str):
        """兼容性方法：根据姓名获取技师信息"""
        return self.repo.get_engineer_by_name(name)

    def get_all_engineers(self, session):
        """兼容性方法：获取所有技师"""
        return self.repo.get_all_engineers()

    def get_all_strengths(self, session):
        """兼容性方法：获取所有技师专长"""
        return self.repo.get_all_strengths()

    def add_engineer(self, session, name: str, gender: Optional[str] = None, strength: Optional[str] = None):
        """兼容性方法：添加技师"""
        return self.repo.add_engineer(name, gender, strength)

    def add_schedule(self, session, engineer_id: int, start_time, end_time, status: str, appointment_id: Optional[int] = None):
        """兼容性方法：添加排班"""
        return self.repo.add_schedule(engineer_id, start_time, end_time, status, appointment_id)

    def get_engineer_schedules(self, session, engineer_id: int, date):
        """兼容性方法：获取技师排班"""
        return self.repo.get_engineer_schedules(engineer_id, date)

    def is_engineer_available(self, session, engineer_id: int, start_time, end_time) -> bool:
        """兼容性方法：检查技师是否可用"""
        return self.repo.is_engineer_available(engineer_id, start_time, end_time)

    def get_engineers_by_gender(self, session, gender: str):
        """兼容性方法：根据性别获取技师"""
        return self.repo.get_engineers_by_gender(gender)


# 为了完全兼容，也提供从原始base_db模块导入的接口
class BaseEngineerDB:
    """兼容性基类"""
    pass
