...
class EngineerFinder:
    ...
    def find_specific_engineer(self, engineer_name: str, start_time: datetime, 
        ...
        if yield_func:
            yield_func(f"[THOUGHT][预约机器人] 用户指定了技师：{engineer_name}，正在查询该技师信息...\n")
        
        specific_tech = appointment_service.get_engineer_by_name(engineer_name)
        if specific_tech:
            if yield_func:
                yield_func(f"[THOUGHT][预约机器人] 找到技师：{specific_tech['name']}，正在检查档期...\n")
            
            if appointment_service.is_engineer_available(specific_tech["id"], start_time, end_time):
                if yield_func:
                    yield_func(f"[THOUGHT][预约机器人] {engineer_name}技师在指定时间有空\n")
                return specific_tech
            else:
                if yield_func:
                    yield_func(f"[THOUGHT][预约机器人] {engineer_name}技师在指定时间不空闲\n")
                return None
        else:
            if yield_func:
                yield_func(f"[THOUGHT][预约机器人] 未找到名为'{engineer_name}'的技师\n")
            return None

    def find_similar_available_engineer(self, target_engineer: Dict[str, Any], 
        ...
        if yield_func:
            yield_func(f"[THOUGHT][预约机器人] 正在根据{target_engineer['name']}的专长查找相似技师...\n")
        
        # 获取所有技师
        all_techs = appointment_service.get_all_engineers()
        if not all_techs:
            return None
            
        # 排除目标技师本身
        other_techs = [tech for tech in all_techs if tech['id'] != target_engineer['id']]
        if not other_techs:
            return None
        
        # 获取目标技师的专长
        target_strength = target_engineer.get('strength', '')
        ...
        for index in indices:
            ...
            if appointment_service.is_engineer_available(similar_tech["id"], start_time, end_time):
                if yield_func:
    ...
    def filter_engineers_by_preference(self, all_techs: list, preference: str) -> list:
        """根据偏好筛选技师"""
    ...
    def filter_engineers_by_gender(self, all_techs: list, gender: str) -> list:
        """根据性别筛选技师"""
    ...
    def find_available_engineer(self, filtered_techs: list, all_techs: list, 
                                start_time: datetime, end_time: datetime, 
        ...
        for tech in filtered_techs:
            if appointment_service.is_engineer_available(tech["id"], start_time, end_time):
                if yield_func:
    ...
    def find_engineer_with_thought(self, appointment_history: Dict[str, Any], 
        ...
        if engineer_name and engineer_name != "未知":
            specific_tech = self.find_specific_engineer(engineer_name, start_time, end_time, yield_func)
            ...
            target_tech = appointment_service.get_engineer_by_name(engineer_name)
            if target_tech:
                similar_tech = self.find_similar_available_engineer(target_tech, start_time, end_time, yield_func)
                if similar_tech:
                    # 返回包含推荐信息的结果，但标记为需要用户确认
                    return {
                        'is_recommendation': True,
                        'original_engineer': target_tech,
                        'recommended_engineer': similar_tech,
        ...
        # 先根据性别筛选技师
        gender_filtered_techs = self.filter_engineers_by_gender(all_techs, gender)
        if yield_func and gender and gender != "未知":
            yield_func(f"[THOUGHT][预约机器人] 根据性别'{gender}'筛选技师，找到{len(gender_filtered_techs)}位技师\n")

        # 再根据偏好筛选技师
        filtered_techs = self.filter_engineers_by_preference(gender_filtered_techs, preference)
        if yield_func and preference and preference != "无":
            yield_func(f"[THOUGHT][预约机器人] 根据偏好'{preference}'进一步筛选，找到{len(filtered_techs)}位技师\n")
        
        # 查找可用技师
        return self.find_available_engineer(filtered_techs, gender_filtered_techs, start_time, end_time, preference, gender, yield_func)
