...
class AppointmentDatabase:
    ...
    def save_appointment(self, engineer_id: str, start_time: datetime, 
        ...
        try:
            ...
            success = self.appointment_service.save_appointment(
                engineer_id, start_time, end_time, appointment_history, session_id
            )
            
            if success:
                # 记录用户行为
                self._record_user_behavior(start_time, end_time, engineer_id, 
    ...
    def update_memory_schedule(self, engineer_id: str, start_time: datetime, end_time: datetime):
        """更新内存中的技师忙碌时间段"""
        busy_period = {
            "start": time_config.format_datetime(start_time, "%H:%M"),
            "end": time_config.format_datetime(end_time, "%H:%M")
        }
        busy_periods_dict.setdefault(engineer_id, []).append(busy_period)
    
    def _record_user_behavior(self, start_time: datetime, end_time: datetime,
                            engineer_id: str, appointment_history: Dict[str, Any], 
                            session_id: str):
        """记录用户预约行为"""
        try:
            action_data = {
