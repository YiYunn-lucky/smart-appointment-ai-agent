    ...
    def analyze_user_preferences(self, user_id: str = "default_user") -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            for appointment in appointments:
                ...
                # 统计技师 - 技师ID存储在单独字段中
                tech_id = appointment.get('engineer_id')  # 从记录本身获取
                if tech_id:
                    engineer_counts[tech_id] = engineer_counts.get(tech_id, 0) + 1
            ...
            # 找出最偏爱的选项
            favorite_engineer = max(engineer_counts, key=engineer_counts.get) if engineer_counts else None
            ...
            return {
                'favorite_engineer_id': favorite_engineer,
    ...
    def generate_return_message(self, user_id: str = "default_user") -> Optional[str]:
        ...
        try:
            ...
            if tech_id:
                ...
                db = EngineerDBRouter()
                tech_info = db.get_engineer_by_id(tech_id)
