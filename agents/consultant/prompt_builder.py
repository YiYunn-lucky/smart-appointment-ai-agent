...
class PromptBuilder:
    ...
    def _create_classification_prompt_template(self) -> str:
        ...
        return (
            ...
            "咨询类问题包括：保修政策、收费标准、故障排查建议、退换货、安装保养、上门服务时间、延保，\n"
            "以及任何查询类请求：查询订单、查询报修单/工单进度、查保修状态、查费用、查热线电话等"
            "（凡是想查安居家电相关信息的问题都是咨询类）。\n"
            "非咨询类问题包括：发起新的报修登记（我要报修、预约师傅上门维修等）、投诉/转人工，\n"
            "以及天气、股票、新闻等完全无关的话题。\n"
            "重要区分：'查询我的报修单/报修进度'是咨询类回答YES；只有'要报修、要预约师傅上门'才是报修登记回答NO。\n"
